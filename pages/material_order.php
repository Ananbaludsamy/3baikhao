...
app.editMaterialOrder = function(orderId) {
    ...
    fetch(`actions/orders/get_order_detail.php?id=${orderId}`)
    .then(r => r.json())
    .then(result => {
...
app.saveMaterialOrder = async function(e) {
    ...
    const isEdit = !!editId;
    const url    = isEdit ? 'actions/orders/update_material_order.php' : 'actions/orders/save_material_order.php';
...
app.loadMaterialOrders = function() {
    ...
    fetch('actions/orders/get_material_orders.php')
    .then(r => r.json())
    .then(result => {
        ...
        return `<tr class="table-row">
            ...
            <td class="text-center">
                <div class="flex justify-center gap-1">
                    ...
                    <button onclick="window.open('pages/print_order.php?id=${o.Order_id}','_blank','width=820,height=700')"
...
app.deleteMaterialOrder = async function(orderId, poNo) {
    ...
    try {
        const res    = await fetch('actions/orders/delete_material_order.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ order_id: orderId }) });
        const result = await res.json();
        if (result.success) {
